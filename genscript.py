...
# Load config from gendoc_config.txt (JSON)
with open('gendoc_config.txt', 'r', encoding='utf-8') as f:
    _gendoc_config = json.load(f)

MAPPING_VALUE_DICT = _gendoc_config['MAPPING_VALUE_DICT']
KOUMOKU_TYPE_MAPPING = _gendoc_config['KOUMOKU_TYPE_MAPPING']
KOUMOKU_TYPE_MAPPING_RE = _gendoc_config['KOUMOKU_TYPE_MAPPING_RE']
STOP_VALUES = set(_gendoc_config['STOP_VALUES'])
EXCLUDED_SHEETNAMES = set(_gendoc_config['EXCLUDED_SHEETNAMES'])
MERGED_CELL_RANGES = {k: tuple(v) for k, v in _gendoc_config['MERGED_CELL_RANGES'].items()}
SKIP_CELL_VALUES = _gendoc_config['SKIP_CELL_VALUES']
ROW_PROCESSOR_CONFIG = _gendoc_config['ROW_PROCESSOR_CONFIG']
LOGIC_PROCESSOR_CONFIG = _gendoc_config['LOGIC_PROCESSOR_CONFIG']
...
def all_tables_in_sequence_with_progress(excel_file, table_info_file, output_file='insert_all.sql', system_id=None, progress_callback=None):
    ...
    for sheet_idx, sheet_name in enumerate(sheetnames):
        if sheet_name in EXCLUDED_SHEETNAMES:
            continue
        sheet_check_value = _sheet_b2_values_cache.get(sheet_name)
        if sheet_check_value in allowed_b2_values:
            valid_sheets.append((sheet_idx, sheet_name, sheet_check_value))
    
    total_sheets = len(valid_sheets)
    current_sheet = 0
    
    for sheet_idx, sheet_name, sheet_check_value in valid_sheets:
        current_sheet += 1
        
        # Update progress
        if progress_callback:
            progress_callback(current_sheet, total_sheets, sheet_name)
        ...
        # Xử lý theo từng loại sheet_check_value
        if sheet_check_value == '項目定義書_帳票':
            re_inserts = re_row(sheet_idx, seq_value)
            all_insert_statements.extend(re_inserts)
        elif sheet_check_value == '項目定義書_CSV':
            csv_inserts = csv_row(sheet_idx, seq_value)
            all_insert_statements.extend(csv_inserts)
        elif sheet_check_value == '項目定義書_IPO図':
            ipo_inserts = ipo_row(sheet_idx, seq_value)
            all_insert_statements.extend(ipo_inserts)
        elif sheet_check_value == '項目定義書_ﾒﾆｭｰ':
            menu_inserts = menu_row(sheet_idx, seq_value)
            all_insert_statements.extend(menu_inserts)
        elif sheet_check_value == '項目定義書_画面':
            koumoku_inserts = koumoku_row(sheet_idx, seq_value)
            all_insert_statements.extend(koumoku_inserts)
            
            func_inserts = func_row(sheet_idx, seq_value)
            all_insert_statements.extend(func_inserts)
            
            message_inserts = message_row(sheet_idx, seq_value)
            all_insert_statements.extend(message_inserts)
            
            tab_inserts = tab_row(sheet_idx, seq_value)
            all_insert_statements.extend(tab_inserts)
            
            ichiran_inserts = ichiran_row(sheet_idx, seq_value)
            all_insert_statements.extend(ichiran_inserts)
            
            hyouji_inserts = hyouji_row(sheet_idx, seq_value)
            all_insert_statements.extend(hyouji_inserts)
